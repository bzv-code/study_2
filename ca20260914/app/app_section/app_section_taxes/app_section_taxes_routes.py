...
async def ndfl_calculator_process(
    ...
    logger.info(f"Расчет НДФЛ: доход={income}, вычеты={deductions}, ежемесячный={is_monthly}")

    try:
        if is_monthly:
            result = logic_ndfl.calculate_monthly_ndfl(income, deductions)
        else:
            result = logic_ndfl.calculate_ndfl(income, deductions)
