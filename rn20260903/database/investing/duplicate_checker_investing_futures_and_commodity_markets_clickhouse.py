...
TABLE_NAME = "investing_futures_and_commodity_markets"


@dataclass
class InvestingFuturesAndCommodityMarketsPipelineStats:
    rss_articles: int = 0
    ...
    def log(self) -> None:
        logger.info("=" * 70)
        logger.info("INVESTING FUTURES AND COMMODITY MARKETS PIPELINE STATISTICS")
        logger.info("=" * 70)
...
@dataclass
class InvestingFuturesAndCommodityMarketsDuplicateCheckResult:
    new_articles: list[Article]
    existing_articles: int
    total_articles: int
    stats: InvestingFuturesAndCommodityMarketsPipelineStats
...
class InvestingFuturesAndCommodityMarketsDuplicateChecker:
    """Проверка существования статей Investing.com Futures and Commodity Markets в ClickHouse."""

    def check_new_articles(
        self,
        articles: list[Article],
    ) -> InvestingFuturesAndCommodityMarketsDuplicateCheckResult:
        stats = InvestingFuturesAndCommodityMarketsPipelineStats()
        stats.set_rss_articles(len(articles))

        if not articles:
            logger.warning("CLICKHOUSE INVESTING FUTURES AND COMMODITY MARKETS DUPLICATE: no articles to check")
            return InvestingFuturesAndCommodityMarketsDuplicateCheckResult(
                new_articles=[],
                existing_articles=0,
                total_articles=0,
                stats=stats,
        ...
        logger.info(
            "CLICKHOUSE INVESTING FUTURES AND COMMODITY MARKETS DUPLICATE: checking %s articles",
            len(articles),
        )
        ...
        logger.info(
            "CLICKHOUSE INVESTING FUTURES AND COMMODITY MARKETS DUPLICATE: RSS articles: %s",
            len(articles),
        )
        logger.info(
            "CLICKHOUSE INVESTING FUTURES AND COMMODITY MARKETS DUPLICATE: already in database: %s",
            existing_count,
        )
        logger.info(
            "CLICKHOUSE INVESTING FUTURES AND COMMODITY MARKETS DUPLICATE: need full description: %s",
            len(new_articles),
        )
    ...
    def update_full_description_statistics(
        ...
        logger.info(
            "CLICKHOUSE INVESTING FUTURES AND COMMODITY MARKETS DUPLICATE: full description received: %s",
            received,
        )
        logger.info(
            "CLICKHOUSE INVESTING FUTURES AND COMMODITY MARKETS DUPLICATE: full description not received: %s",
            not_received,
        )
    ...
    def update_insert_statistics(
        ...
        logger.info(
            "CLICKHOUSE INVESTING FUTURES AND COMMODITY MARKETS DUPLICATE: inserted into ClickHouse: %s",
            inserted_count,
        )
    ...
    def _get_existing_urls(self, articles: list[Article]) -> set[str]:
        ...
        logger.debug("CLICKHOUSE INVESTING FUTURES AND COMMODITY MARKETS DUPLICATE: checking URLs in database")
        ...
        try:
            ...
            if not result or not hasattr(result, 'result_rows') or result.result_rows is None:
                logger.warning(
                    "CLICKHOUSE INVESTING FUTURES AND COMMODITY MARKETS DUPLICATE: query returned no result"
                )
                return set()
            ...
            logger.error(
                "CLICKHOUSE INVESTING FUTURES AND COMMODITY MARKETS DUPLICATE: failed to query database: %s",
                str(e),
            )
            return set()
