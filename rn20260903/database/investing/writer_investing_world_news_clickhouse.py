...
class InvestingWorldNewsClickHouseWriter:
    """Writer для записи статей Investing.com World News в ClickHouse."""
    ...
    def write(self, articles: list[Article]) -> int:
        if not articles:
            logger.warning("CLICKHOUSE INVESTING WORLD NEWS: no articles to write")
            return 0

        logger.info(
            "CLICKHOUSE INVESTING WORLD NEWS: starting write: %s articles",
            len(articles),
        )
        ...
        if skipped_count:
            logger.warning(
                "CLICKHOUSE INVESTING WORLD NEWS: skipped %s articles without description_full",
                skipped_count,
            )

        if not valid_articles:
            logger.warning("CLICKHOUSE INVESTING WORLD NEWS: no valid articles to write")
            return 0

        logger.info(
            "CLICKHOUSE INVESTING WORLD NEWS: valid articles: %s",
            len(valid_articles),
        )
        ...
        for article in valid_articles:
            try:
                ...
                conversion_errors += 1
                logger.warning(
                    "CLICKHOUSE INVESTING WORLD NEWS: article skipped: %s | reason: %s",
                    article.url, exc,
                )

        if conversion_errors:
            logger.warning(
                "CLICKHOUSE INVESTING WORLD NEWS: %s articles skipped during conversion",
                conversion_errors,
            )

        if not rows:
            logger.warning("CLICKHOUSE INVESTING WORLD NEWS: no rows to write")
            return 0
        ...
        with ClickHouseClient() as client:
            for batch_start in range(0, len(rows), self.batch_size):
                ...
                logger.info(
                    "CLICKHOUSE INVESTING WORLD NEWS: batch written: %s-%s/%s",
        ...
        logger.info(
            "CLICKHOUSE INVESTING WORLD NEWS: write finished: %s articles",
            total_written,
        )
