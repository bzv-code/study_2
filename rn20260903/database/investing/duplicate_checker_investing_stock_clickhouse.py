...
@dataclass
class InvestingStockPipelineStats:
    """Статистика обработки Investing.com Stock RSS."""
    ...
    def log(self) -> None:
        logger.info("=" * 70)
        logger.info("INVESTING STOCK PIPELINE STATISTICS")
        logger.info("=" * 70)
...
@dataclass
class InvestingStockDuplicateCheckResult:
    new_articles: list[Article]
    existing_articles: int
    total_articles: int
    stats: InvestingStockPipelineStats
...
class InvestingStockDuplicateChecker:
    """Проверка существования статей Investing.com Stock в ClickHouse."""

    def check_new_articles(
        self,
        articles: list[Article],
    ) -> InvestingStockDuplicateCheckResult:
        stats = InvestingStockPipelineStats()
        stats.set_rss_articles(len(articles))

        if not articles:
            logger.warning("CLICKHOUSE INVESTING STOCK DUPLICATE: no articles to check")
            return InvestingStockDuplicateCheckResult(
                new_articles=[],
                existing_articles=0,
                total_articles=0,
                stats=stats,
        ...
        logger.info(
            "CLICKHOUSE INVESTING STOCK DUPLICATE: checking %s articles",
            len(articles),
        )
        ...
        logger.info(
            "CLICKHOUSE INVESTING STOCK DUPLICATE: RSS articles: %s",
            len(articles),
        )
        logger.info(
            "CLICKHOUSE INVESTING STOCK DUPLICATE: already in database: %s",
            existing_count,
        )
        logger.info(
            "CLICKHOUSE INVESTING STOCK DUPLICATE: need full description: %s",
            len(new_articles),
        )
    ...
    def update_full_description_statistics(
        ...
        logger.info(
            "CLICKHOUSE INVESTING STOCK DUPLICATE: full description received: %s",
            received,
        )
        logger.info(
            "CLICKHOUSE INVESTING STOCK DUPLICATE: full description not received: %s",
            not_received,
        )
    ...
    def update_insert_statistics(
        ...
        logger.info(
            "CLICKHOUSE INVESTING STOCK DUPLICATE: inserted into ClickHouse: %s",
            inserted_count,
        )
    ...
    def _get_existing_urls(self, articles: list[Article]) -> set[str]:
        ...
        logger.debug("CLICKHOUSE INVESTING STOCK DUPLICATE: checking URLs in database")
        ...
        try:
            ...
            if not result or not hasattr(result, 'result_rows') or result.result_rows is None:
                logger.warning(
                    "CLICKHOUSE INVESTING STOCK DUPLICATE: query returned no result"
                )
                return set()
            ...
            logger.error(
                "CLICKHOUSE INVESTING STOCK DUPLICATE: failed to query database: %s",
                str(e),
            )
            return set()
