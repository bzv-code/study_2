...
class InvestingCommoditiesClickHouseWriter:
    """Writer для записи статей Investing.com Commodities в ClickHouse."""
    ...
    def write(self, articles: list[Article]) -> int:
        if not articles:
            logger.warning("CLICKHOUSE INVESTING COMMODITIES: no articles to write")
            return 0

        logger.info(
            "CLICKHOUSE INVESTING COMMODITIES: starting write: %s articles",
            len(articles),
        )
        ...
        if skipped_count:
            logger.warning(
                "CLICKHOUSE INVESTING COMMODITIES: skipped %s articles without description_full",
                skipped_count,
            )

        if not valid_articles:
            logger.warning("CLICKHOUSE INVESTING COMMODITIES: no valid articles to write")
            return 0

        logger.info(
            "CLICKHOUSE INVESTING COMMODITIES: valid articles: %s",
            len(valid_articles),
        )
        ...
        for article in valid_articles:
            try:
                ...
                conversion_errors += 1
                logger.warning(
                    "CLICKHOUSE INVESTING COMMODITIES: article skipped: %s | reason: %s",
                    article.url, exc,
                )

        if conversion_errors:
            logger.warning(
                "CLICKHOUSE INVESTING COMMODITIES: %s articles skipped during conversion",
                conversion_errors,
            )

        if not rows:
            logger.warning("CLICKHOUSE INVESTING COMMODITIES: no rows to write")
            return 0
        ...
        with ClickHouseClient() as client:
            for batch_start in range(0, len(rows), self.batch_size):
                ...
                logger.info(
                    "CLICKHOUSE INVESTING COMMODITIES: batch written: %s-%s/%s",
        ...
        logger.info(
            "CLICKHOUSE INVESTING COMMODITIES: write finished: %s articles",
            total_written,
        )
