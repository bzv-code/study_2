...
TABLE_NAME = "investing_forms_sec"


@dataclass
class InvestingFormsSecPipelineStats:
    rss_articles: int = 0
    ...
    def log(self) -> None:
        logger.info("=" * 70)
        logger.info("INVESTING FORMS SEC PIPELINE STATISTICS")
        logger.info("=" * 70)
...
@dataclass
class InvestingFormsSecDuplicateCheckResult:
    new_articles: list[Article]
    existing_articles: int
    total_articles: int
    stats: InvestingFormsSecPipelineStats
...
class InvestingFormsSecDuplicateChecker:
    """Проверка существования статей Investing.com Forms SEC в ClickHouse."""

    def check_new_articles(
        self,
        articles: list[Article],
    ) -> InvestingFormsSecDuplicateCheckResult:
        stats = InvestingFormsSecPipelineStats()
        stats.set_rss_articles(len(articles))

        if not articles:
            logger.warning("CLICKHOUSE INVESTING FORMS SEC DUPLICATE: no articles to check")
            return InvestingFormsSecDuplicateCheckResult(
                new_articles=[],
                existing_articles=0,
                total_articles=0,
                stats=stats,
        ...
        logger.info(
            "CLICKHOUSE INVESTING FORMS SEC DUPLICATE: checking %s articles",
            len(articles),
        )
        ...
        logger.info(
            "CLICKHOUSE INVESTING FORMS SEC DUPLICATE: RSS articles: %s",
            len(articles),
        )
        logger.info(
            "CLICKHOUSE INVESTING FORMS SEC DUPLICATE: already in database: %s",
            existing_count,
        )
        logger.info(
            "CLICKHOUSE INVESTING FORMS SEC DUPLICATE: need full description: %s",
            len(new_articles),
        )
    ...
    def update_full_description_statistics(
        ...
        logger.info(
            "CLICKHOUSE INVESTING FORMS SEC DUPLICATE: full description received: %s",
            received,
        )
        logger.info(
            "CLICKHOUSE INVESTING FORMS SEC DUPLICATE: full description not received: %s",
            not_received,
        )
    ...
    def update_insert_statistics(
        ...
        logger.info(
            "CLICKHOUSE INVESTING FORMS SEC DUPLICATE: inserted into ClickHouse: %s",
            inserted_count,
        )
    ...
    def _get_existing_urls(self, articles: list[Article]) -> set[str]:
        ...
        logger.debug("CLICKHOUSE INVESTING FORMS SEC DUPLICATE: checking URLs in database")
        ...
        try:
            ...
            if not result or not hasattr(result, 'result_rows') or result.result_rows is None:
                logger.warning(
                    "CLICKHOUSE INVESTING FORMS SEC DUPLICATE: query returned no result"
                )
                return set()
            ...
            logger.error(
                "CLICKHOUSE INVESTING FORMS SEC DUPLICATE: failed to query database: %s",
                str(e),
            )
            return set()
