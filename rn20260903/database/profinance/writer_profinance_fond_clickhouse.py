...
class ProFinanceFondClickHouseWriter:
    """
    Writer для записи статей ProFinance Fond в ClickHouse.
    ...
    def write(self, articles: list[Article]) -> int:
        """
        Записать статьи ProFinance Fond в ClickHouse.
        """

        if not articles:
            logger.warning("CLICKHOUSE PROFINANCE FOND: no articles to write")
            return 0

        logger.info(
            "CLICKHOUSE PROFINANCE FOND: starting write: %s articles",
            len(articles),
        )
        ...
        if skipped_count:
            logger.warning(
                "CLICKHOUSE PROFINANCE FOND: skipped %s articles without description_full",
                skipped_count,
            )

        if not valid_articles:
            logger.warning("CLICKHOUSE PROFINANCE FOND: no valid articles to write")
            return 0

        logger.info(
            "CLICKHOUSE PROFINANCE FOND: valid articles: %s",
            len(valid_articles),
        )
        ...
        for article in valid_articles:
            try:
                ...
                conversion_errors += 1
                logger.warning(
                    "CLICKHOUSE PROFINANCE FOND: article skipped: %s | reason: %s",
                    article.url,
                    exc,
                )
        ...
        if conversion_errors:
            logger.warning(
                "CLICKHOUSE PROFINANCE FOND: %s articles skipped during conversion",
                conversion_errors,
            )

        if not rows:
            logger.warning("CLICKHOUSE PROFINANCE FOND: no rows to write")
            return 0
        ...
        with ClickHouseClient() as client:
            for batch_start in range(0, len(rows), self.batch_size):
                ...
                logger.info(
                    "CLICKHOUSE PROFINANCE FOND: batch written: %s-%s/%s",
        ...
        logger.info(
            "CLICKHOUSE PROFINANCE FOND: write finished: %s articles",
            total_written,
        )
