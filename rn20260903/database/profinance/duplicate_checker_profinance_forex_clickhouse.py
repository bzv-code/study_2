...
@dataclass
class ProFinancePipelineStats:
    """
    Статистика обработки ProFinance RSS.
    """
    ...
    def log(self) -> None:
        logger.info("=" * 70)
        logger.info("PROFINANCE PIPELINE STATISTICS")
        logger.info("=" * 70)
...
@dataclass
class ProFinanceDuplicateCheckResult:
    """
...
class ProFinanceForexDuplicateChecker:
    """
    Проверка существования статей ProFinance в ClickHouse.
    """

    def check_new_articles(
        self,
        articles: list[Article],
    ) -> ProFinanceDuplicateCheckResult:
        """
        ...
        if not articles:
            logger.warning("CLICKHOUSE PROFINANCE DUPLICATE: no articles to check")
            return ProFinanceDuplicateCheckResult(
                new_articles=[],
                existing_articles=0,
                total_articles=0,
                stats=stats,
        ...
        logger.info(
            "CLICKHOUSE PROFINANCE DUPLICATE: checking %s articles",
            len(articles),
        )
        ...
        logger.info(
            "CLICKHOUSE PROFINANCE DUPLICATE: RSS articles: %s",
            len(articles),
        )
        logger.info(
            "CLICKHOUSE PROFINANCE DUPLICATE: already in database: %s",
            existing_count,
        )
        logger.info(
            "CLICKHOUSE PROFINANCE DUPLICATE: need full description: %s",
            len(new_articles),
        )
    ...
    def update_full_description_statistics(
        ...
        logger.info(
            "CLICKHOUSE PROFINANCE DUPLICATE: full description received: %s",
            received,
        )
        logger.info(
            "CLICKHOUSE PROFINANCE DUPLICATE: full description not received: %s",
            not_received,
        )
    ...
    def update_insert_statistics(
        ...
        logger.info(
            "CLICKHOUSE PROFINANCE DUPLICATE: inserted into ClickHouse: %s",
            inserted_count,
        )
    ...
    def _get_existing_urls(self, articles: list[Article]) -> set[str]:
        ...
        logger.debug("CLICKHOUSE PROFINANCE DUPLICATE: checking URLs in database")
