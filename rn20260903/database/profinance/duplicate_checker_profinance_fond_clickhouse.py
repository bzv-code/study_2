...
@dataclass
class ProFinanceFondPipelineStats:
    """
    Статистика обработки ProFinance Fond RSS.
    """
    ...
    def log(self) -> None:
        logger.info("=" * 70)
        logger.info("PROFINANCE FOND PIPELINE STATISTICS")
        logger.info("=" * 70)
...
@dataclass
class ProFinanceFondDuplicateCheckResult:
    """
    ...
    existing_articles: int
    total_articles: int
    stats: ProFinanceFondPipelineStats
...
class ProFinanceFondDuplicateChecker:
    """
    Проверка существования статей ProFinance Fond в ClickHouse.
    """

    def check_new_articles(
        self,
        articles: list[Article],
    ) -> ProFinanceFondDuplicateCheckResult:
        """
        ...
        if not articles:
            logger.warning("CLICKHOUSE PROFINANCE FOND DUPLICATE: no articles to check")
            return ProFinanceFondDuplicateCheckResult(
                new_articles=[],
                existing_articles=0,
                total_articles=0,
                stats=stats,
        ...
        logger.info(
            "CLICKHOUSE PROFINANCE FOND DUPLICATE: checking %s articles",
            len(articles),
        )
        ...
        logger.info(
            "CLICKHOUSE PROFINANCE FOND DUPLICATE: RSS articles: %s",
            len(articles),
        )
        logger.info(
            "CLICKHOUSE PROFINANCE FOND DUPLICATE: already in database: %s",
            existing_count,
        )
        logger.info(
            "CLICKHOUSE PROFINANCE FOND DUPLICATE: need full description: %s",
            len(new_articles),
        )
    ...
    def update_full_description_statistics(
        ...
        logger.info(
            "CLICKHOUSE PROFINANCE FOND DUPLICATE: full description received: %s",
            received,
        )
        logger.info(
            "CLICKHOUSE PROFINANCE FOND DUPLICATE: full description not received: %s",
            not_received,
        )
    ...
    def update_insert_statistics(
        ...
        logger.info(
            "CLICKHOUSE PROFINANCE FOND DUPLICATE: inserted into ClickHouse: %s",
            inserted_count,
        )
    ...
    def _get_existing_urls(self, articles: list[Article]) -> set[str]:
        ...
        logger.debug("CLICKHOUSE PROFINANCE FOND DUPLICATE: checking URLs in database")
        ...
        try:
            ...
            if not result or not hasattr(result, 'result_rows') or result.result_rows is None:
                logger.warning(
                    "CLICKHOUSE PROFINANCE FOND DUPLICATE: query returned no result"
                )
                return set()
            ...
            logger.error(
                "CLICKHOUSE PROFINANCE FOND DUPLICATE: failed to query database: %s",
                str(e),
            )
            # Возвращаем пустое множество, чтобы все статьи считались новыми
            return set()
