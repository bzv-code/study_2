...
@dataclass
class ProFinanceEconomPipelineStats:
    """
    Статистика обработки ProFinance Econom RSS.
    """
    ...
    def log(self) -> None:
        logger.info("=" * 70)
        logger.info("PROFINANCE ECONOM PIPELINE STATISTICS")
        logger.info("=" * 70)
...
@dataclass
class ProFinanceEconomDuplicateCheckResult:
    """
    ...
    existing_articles: int
    total_articles: int
    stats: ProFinanceEconomPipelineStats
...
class ProFinanceEconomDuplicateChecker:
    """
    Проверка существования статей ProFinance Econom в ClickHouse.
    """

    def check_new_articles(
        self,
        articles: list[Article],
    ) -> ProFinanceEconomDuplicateCheckResult:
        """
        ...
        if not articles:
            logger.warning("CLICKHOUSE PROFINANCE ECONOM DUPLICATE: no articles to check")
            return ProFinanceEconomDuplicateCheckResult(
                new_articles=[],
                existing_articles=0,
                total_articles=0,
                stats=stats,
        ...
        logger.info(
            "CLICKHOUSE PROFINANCE ECONOM DUPLICATE: checking %s articles",
            len(articles),
        )
        ...
        logger.info(
            "CLICKHOUSE PROFINANCE ECONOM DUPLICATE: RSS articles: %s",
            len(articles),
        )
        logger.info(
            "CLICKHOUSE PROFINANCE ECONOM DUPLICATE: already in database: %s",
            existing_count,
        )
        logger.info(
            "CLICKHOUSE PROFINANCE ECONOM DUPLICATE: need full description: %s",
            len(new_articles),
        )
    ...
    def update_full_description_statistics(
        ...
        logger.info(
            "CLICKHOUSE PROFINANCE ECONOM DUPLICATE: full description received: %s",
            received,
        )
        logger.info(
            "CLICKHOUSE PROFINANCE ECONOM DUPLICATE: full description not received: %s",
            not_received,
        )
    ...
    def update_insert_statistics(
        ...
        logger.info(
            "CLICKHOUSE PROFINANCE ECONOM DUPLICATE: inserted into ClickHouse: %s",
            inserted_count,
        )
    ...
    def _get_existing_urls(self, articles: list[Article]) -> set[str]:
        ...
        logger.debug("CLICKHOUSE PROFINANCE ECONOM DUPLICATE: checking URLs in database")
