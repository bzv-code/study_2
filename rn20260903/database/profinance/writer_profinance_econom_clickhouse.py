...
class ProFinanceEconomClickHouseWriter:
    """
    Writer для записи статей ProFinance Econom в ClickHouse.
    ...
    def write(self, articles: list[Article]) -> int:
        """
        Записать статьи ProFinance Econom в ClickHouse.
        """

        if not articles:
            logger.warning("CLICKHOUSE PROFINANCE ECONOM: no articles to write")
            return 0

        logger.info(
            "CLICKHOUSE PROFINANCE ECONOM: starting write: %s articles",
            len(articles),
        )
        ...
        if skipped_count:
            logger.warning(
                "CLICKHOUSE PROFINANCE ECONOM: skipped %s articles without description_full",
                skipped_count,
            )

        if not valid_articles:
            logger.warning("CLICKHOUSE PROFINANCE ECONOM: no valid articles to write")
            return 0

        logger.info(
            "CLICKHOUSE PROFINANCE ECONOM: valid articles: %s",
            len(valid_articles),
        )
        ...
        for article in valid_articles:
            try:
                ...
                conversion_errors += 1
                logger.warning(
                    "CLICKHOUSE PROFINANCE ECONOM: article skipped: %s | reason: %s",
                    article.url,
                    exc,
                )
        ...
        if conversion_errors:
            logger.warning(
                "CLICKHOUSE PROFINANCE ECONOM: %s articles skipped during conversion",
                conversion_errors,
            )

        if not rows:
            logger.warning("CLICKHOUSE PROFINANCE ECONOM: no rows to write")
            return 0
        ...
        with ClickHouseClient() as client:
            for batch_start in range(0, len(rows), self.batch_size):
                ...
                logger.info(
                    "CLICKHOUSE PROFINANCE ECONOM: batch written: %s-%s/%s",
        ...
        logger.info(
            "CLICKHOUSE PROFINANCE ECONOM: write finished: %s articles",
            total_written,
        )
