...
class ProFinanceFondArticleService:
    """
    Сервис получения метатегов и полного текста статьи ProFinance Fond.
    """
    ...
    # Отдельный профиль браузера для Fond RSS
    PROFILE_DIR = (
        PROJECT_ROOT
        / "data"
        / "browser_profiles"
    ...
    async def enrich_articles(
        ...
        if not articles:
            logger.warning("PROFINANCE FOND ARTICLE: no articles to process")
            return articles

        logger.info(
            "PROFINANCE FOND ARTICLE: starting parsing: %s articles",
            len(articles),
        )
        ...
        async with async_playwright() as playwright:
            ...
            try:
                ...
                for index, article in enumerate(articles, start=1):
                    logger.info(
                        "PROFINANCE FOND ARTICLE: processing %s/%s: %s",
                        ...
                        if article_data:
                            ...
                            if article_data.get("published_at_raw"):
                                try:
                                    ...
                                    logger.warning(
                                        "PROFINANCE FOND ARTICLE: invalid date format: %s",
                                        article_data.get("published_at_raw"),
                                    )

                            logger.info(
                                "PROFINANCE FOND ARTICLE: data received: %s/%s, %s chars",
                                index, len(articles),
                                len(article_data.get("description_full", "")),
                            )
                        else:
                            logger.warning(
                                "PROFINANCE FOND ARTICLE: data not found: %s/%s",
                                index, len(articles),
                            )

                    except Exception:
                        logger.exception(
                            "PROFINANCE FOND ARTICLE: failed to process %s/%s: %s",
                            index, len(articles), article.url,
                        )
    ...
    async def _fetch_article_data(
        ...
        if response is not None and response.status >= 400:
            logger.warning(
                "PROFINANCE FOND ARTICLE: HTTP %s received: %s",
                response.status, url,
            )
            return None
        ...
        try:
            ...
            logger.warning("PROFINANCE FOND ARTICLE: container not found: %s", url)
            return None
        ...
        try:
            ...
            logger.exception("PROFINANCE FOND ARTICLE: failed to get text: %s", url)
