...
class InvestingCurrencyMarketArticleService:
    """
    Сервис получения полного текста статьи Investing.com Currency Market.
    """
    ...
    DEBUG_DIR = Path(__file__).resolve().parent.parent.parent / "data" / "debug" / "investing_currency_market"
    ...
    async def enrich_articles(
        ...
        if not articles:
            logger.warning("INVESTING CURRENCY MARKET ARTICLE: no articles to process")
            return articles

        if not CURL_CFFI_AVAILABLE:
            logger.error("INVESTING CURRENCY MARKET ARTICLE: curl_cffi не установлен")
            return articles

        logger.info(
            "INVESTING CURRENCY MARKET ARTICLE: starting parsing: %s articles",
            len(articles),
        )
        ...
        for index, article in enumerate(articles, start=1):
            logger.info(
                "INVESTING CURRENCY MARKET ARTICLE: processing %s/%s: %s",
            ...
            try:
                ...
                if article_data:
                    ...
                    logger.info(
                        "INVESTING CURRENCY MARKET ARTICLE: data received: %s/%s, %s chars",
                        index, len(articles), full_len,
                    )
                else:
                    logger.warning(
                        "INVESTING CURRENCY MARKET ARTICLE: data not found: %s/%s",
                        index, len(articles),
                    )

            except Exception:
                logger.exception(
                    "INVESTING CURRENCY MARKET ARTICLE: failed to process %s/%s: %s",
                    index, len(articles), article.url,
                )
        ...
        try:
            with curl_requests.Session(impersonate=self.IMPERSONATE_PROFILE) as session:
                ...
                if response.status_code >= 400:
                    logger.warning(
                        "INVESTING CURRENCY MARKET ARTICLE: HTTP %s for %s",
                ...
                logger.info(
                    "INVESTING CURRENCY MARKET ARTICLE: HTML size=%d bytes, status=%d",
                ...
                if save_debug:
                    ...
                    logger.info(
                        "INVESTING CURRENCY MARKET ARTICLE: debug HTML saved: %s",
                        debug_file,
                    )

                if len(html) < 5000:
                    logger.warning(
                        "INVESTING CURRENCY MARKET ARTICLE: HTML слишком короткий (%d байт)",
                        len(html),
                    )
                    return None
            ...
            logger.exception(
                "INVESTING CURRENCY MARKET ARTICLE: sync fetch failed for %s: %s",
                url, e,
            )
            return None
    ...
    def _parse_html(self, html: str) -> dict | None:
        ...
        if next_data:
            ...
            if article_json:
                ...
                if body_html:
                    ...
                    if cleaned:
                        # Применяем investing-specific очистку к полному тексту
                        cleaned = self._clean_investing_text(cleaned)
                        result["description_full"] = cleaned

                        # Извлекаем первый абзац и очищаем его
        ...
        # =====================================================================
        if not result.get("description_full"):
            logger.info("INVESTING CURRENCY MARKET ARTICLE: trying HTML fallback parsing")
        ...
        # =====================================================================
        # Title - с investing-specific очисткой
        # =====================================================================
        ...
        if og_title_match:
            ...
            # Сначала убираем стандартный суффикс " | Investing.com"
            title = re.sub(r'\s*\|\s*Investing\.com\s*$', '', title)
            # Затем применяем investing-specific очистку (" От Investing.com" и т.д.)
    ...
    def _extract_next_data(self, html: str) -> dict | None:
        ...
        if not match:
            logger.warning("INVESTING CURRENCY MARKET ARTICLE: __NEXT_DATA__ не найден")
            return None

        try:
            return json.loads(match.group(1).strip())
        except json.JSONDecodeError as e:
            logger.warning("INVESTING CURRENCY MARKET ARTICLE: ошибка парсинга __NEXT_DATA__: %s", e)
            return None
    ...
    def _get_article_from_next_data(self, next_data: dict) -> dict | None:
        ...
        logger.info(
            "INVESTING CURRENCY MARKET ARTICLE: pageProps keys: %s",
        ...
        if isinstance(analysis_store, dict):
            logger.debug(
                "INVESTING CURRENCY MARKET ARTICLE: analysisStore keys: %s",
            ...
            if isinstance(article, dict):
                logger.debug(
                    "INVESTING CURRENCY MARKET ARTICLE: _article keys: %s",
                    list(article.keys()),
                )

                if article.get("body"):
                    logger.info("INVESTING CURRENCY MARKET ARTICLE: found via analysisStore._article.body")
                    return article

        # Путь 2: Рекурсивный поиск
        for key in page_props.keys():
            ...
            if "_article" in val and isinstance(val["_article"], dict):
                ...
                if article.get("body"):
                    logger.info(
                        "INVESTING CURRENCY MARKET ARTICLE: found via pageProps.%s._article.body",
                        key,
                    )
                    return article
            ...
            if "article" in val and isinstance(val["article"], dict):
                ...
                if article.get("body"):
                    logger.info(
                        "INVESTING CURRENCY MARKET ARTICLE: found via pageProps.%s.article.body",
                        key,
                    )
                    return article
        ...
        for key in direct_keys:
            ...
            if isinstance(val, dict) and val.get("body"):
                logger.info(
                    "INVESTING CURRENCY MARKET ARTICLE: found via pageProps.%s.body",
                    key,
                )
                return val
        ...
        for key1, val1 in page_props.items():
            ...
            for key2, val2 in val1.items():
                ...
                if val2.get("body"):
                    logger.info(
                        "INVESTING CURRENCY MARKET ARTICLE: found via pageProps.%s.%s.body",
                        key1, key2,
                    )
                    return val2
                ...
                for key3 in ["_article", "article"]:
                    ...
                    if isinstance(val3, dict) and val3.get("body"):
                        logger.info(
                            "INVESTING CURRENCY MARKET ARTICLE: found via pageProps.%s.%s.%s.body",
                            key1, key2, key3,
                        )
                        return val3

        logger.warning(
            "INVESTING CURRENCY MARKET ARTICLE: не удалось найти body в __NEXT_DATA__. "
    ...
    def _parse_html_fallback(self, html: str) -> dict:
        ...
        if match:
            ...
            logger.info("INVESTING CURRENCY MARKET ARTICLE: found article via id='article'")
        ...
        if not article_html:
            ...
            for m in matches:
                if len(m) > 1000:
                    article_html = m
                    logger.info("INVESTING CURRENCY MARKET ARTICLE: found article via class with 'article'")
                    break
    ...
    def _clean_html_body(html_body: str) -> str:
        """
        Базовая очистка HTML body:
            1. Декодировать HTML-сущности
            2. Заменить <br>, <p> на переносы строк
            3. Удалить все остальные теги
            4. Сжать пробелы
            5. Убрать пустые строки
        """
        if not html_body:
            return ""
    ...
    def _get_first_paragraph(text: str) -> str:
        """Получить первый непустой абзац как description."""
        if not text:
            return ""
    ...
    # =========================================================================
    # INVESTING-SPECIFIC CLEANING (специфичная очистка для Investing.com)
    # =========================================================================
    ...
    def _parse_iso_date(date_str: str) -> datetime | None:
        """Парсить дату ISO 8601: 2026-08-12T12:05:19.000+00:00"""
        if not date_str:
            return None
        ...
        try:
            ...
            logger.warning(
                "INVESTING CURRENCY MARKET ARTICLE: не удалось распарсить дату '%s': %s",
                date_str, e,
            )
            return None