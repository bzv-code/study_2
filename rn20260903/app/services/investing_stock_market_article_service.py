...
class InvestingStockMarketArticleService:
    """
    Сервис получения полного текста статьи Investing.com Stock Market.
    """
    ...
    DEBUG_DIR = Path(__file__).resolve().parent.parent.parent / "data" / "debug" / "investing_stock_market"
    ...
    async def enrich_articles(
        ...
        if not articles:
            logger.warning("INVESTING STOCK MARKET ARTICLE: no articles to process")
            return articles

        if not CURL_CFFI_AVAILABLE:
            logger.error("INVESTING STOCK MARKET ARTICLE: curl_cffi не установлен")
            return articles

        logger.info(
            "INVESTING STOCK MARKET ARTICLE: starting parsing: %s articles",
            len(articles),
        )
        ...
        for index, article in enumerate(articles, start=1):
            logger.info(
                "INVESTING STOCK MARKET ARTICLE: processing %s/%s: %s",
            ...
            try:
                ...
                if article_data:
                    ...
                    logger.info(
                        "INVESTING STOCK MARKET ARTICLE: data received: %s/%s, %s chars",
                        index, len(articles), full_len,
                    )
                else:
                    logger.warning(
                        "INVESTING STOCK MARKET ARTICLE: data not found: %s/%s",
                        index, len(articles),
                    )

            except Exception:
                logger.exception(
                    "INVESTING STOCK MARKET ARTICLE: failed to process %s/%s: %s",
                    index, len(articles), article.url,
                )
        ...
        try:
            with curl_requests.Session(impersonate=self.IMPERSONATE_PROFILE) as session:
                ...
                if response.status_code >= 400:
                    logger.warning(
                        "INVESTING STOCK MARKET ARTICLE: HTTP %s for %s",
                ...
                logger.info(
                    "INVESTING STOCK MARKET ARTICLE: HTML size=%d bytes, status=%d",
                ...
                if save_debug:
                    ...
                    logger.info(
                        "INVESTING STOCK MARKET ARTICLE: debug HTML saved: %s",
                        debug_file,
                    )

                if len(html) < 5000:
                    logger.warning(
                        "INVESTING STOCK MARKET ARTICLE: HTML слишком короткий (%d байт)",
                        len(html),
                    )
                    return None
            ...
            logger.exception(
                "INVESTING STOCK MARKET ARTICLE: sync fetch failed for %s: %s",
                url, e,
            )
            return None
    ...
    def _parse_html(self, html: str) -> dict | None:
        ...
        # =====================================================================
        if not result.get("description_full"):
            logger.info("INVESTING STOCK MARKET ARTICLE: trying HTML fallback parsing")
    ...
    def _extract_next_data(self, html: str) -> dict | None:
        ...
        if not match:
            logger.warning("INVESTING STOCK MARKET ARTICLE: __NEXT_DATA__ не найден")
            return None

        try:
            return json.loads(match.group(1).strip())
        except json.JSONDecodeError as e:
            logger.warning("INVESTING STOCK MARKET ARTICLE: ошибка парсинга __NEXT_DATA__: %s", e)
            return None
    ...
    def _get_article_from_next_data(self, next_data: dict) -> dict | None:
        ...
        logger.info(
            "INVESTING STOCK MARKET ARTICLE: pageProps keys: %s",
        ...
        if isinstance(analysis_store, dict):
            logger.debug(
                "INVESTING STOCK MARKET ARTICLE: analysisStore keys: %s",
            ...
            if isinstance(article, dict):
                logger.debug(
                    "INVESTING STOCK MARKET ARTICLE: _article keys: %s",
                    list(article.keys()),
                )

                if article.get("body"):
                    logger.info("INVESTING STOCK MARKET ARTICLE: found via analysisStore._article.body")
                    return article
        ...
        for key in page_props.keys():
            ...
            if "_article" in val and isinstance(val["_article"], dict):
                ...
                if article.get("body"):
                    logger.info(
                        "INVESTING STOCK MARKET ARTICLE: found via pageProps.%s._article.body",
                        key,
                    )
                    return article
            ...
            if "article" in val and isinstance(val["article"], dict):
                ...
                if article.get("body"):
                    logger.info(
                        "INVESTING STOCK MARKET ARTICLE: found via pageProps.%s.article.body",
                        key,
                    )
                    return article
        ...
        for key in direct_keys:
            ...
            if isinstance(val, dict) and val.get("body"):
                logger.info(
                    "INVESTING STOCK MARKET ARTICLE: found via pageProps.%s.body",
                    key,
                )
                return val
        ...
        for key1, val1 in page_props.items():
            ...
            for key2, val2 in val1.items():
                ...
                if val2.get("body"):
                    logger.info(
                        "INVESTING STOCK MARKET ARTICLE: found via pageProps.%s.%s.body",
                        key1, key2,
                    )
                    return val2
                ...
                for key3 in ["_article", "article"]:
                    ...
                    if isinstance(val3, dict) and val3.get("body"):
                        logger.info(
                            "INVESTING STOCK MARKET ARTICLE: found via pageProps.%s.%s.%s.body",
                            key1, key2, key3,
                        )
                        return val3

        logger.warning(
            "INVESTING STOCK MARKET ARTICLE: не удалось найти body в __NEXT_DATA__. "
    ...
    def _parse_html_fallback(self, html: str) -> dict:
        ...
        if match:
            ...
            logger.info("INVESTING STOCK MARKET ARTICLE: found article via id='article'")
        ...
        if not article_html:
            ...
            for m in matches:
                if len(m) > 1000:
                    article_html = m
                    logger.info("INVESTING STOCK MARKET ARTICLE: found article via class with 'article'")
                    break
    ...
    def _parse_iso_date(date_str: str) -> datetime | None:
        ...
        try:
            ...
            logger.warning(
                "INVESTING STOCK MARKET ARTICLE: не удалось распарсить дату '%s': %s",
                date_str, e,
            )
            return None